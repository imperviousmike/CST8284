...
public class ComplexCalculatorLauncher {
	...
	private static Complex getComplexNumber() {
		...
		// bonus B
		System.out.format("Enter the %s complex number ", (count++ % 2 == 0) ? "first" : "second");
	...
	// Standard menu method; prompts user to enter input type for Complex
	// constructor
	private static int displayTypeInputMenu() {
		System.out.printf("%s\n\t%s\n\t%s\n\t%s", "Enter the format of the complex number to be entered:",
				SINGLE_STRING + ". A single string", DOUBLE_STRING + ". Two strings",
